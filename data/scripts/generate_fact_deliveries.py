...
def generate_rows(total_rows: int = 1000) -> list[dict[str, object]]:
    ...
    hubs = read_csv_rows("dim_hubs.csv") # Carrega os Hubs para obter coordenadas de origem
    
    # Cria um mapeamento de Hubs para busca rápida de latitude/longitude de origem
    hubs_map = {h["hub_id"]: {"lat": h["latitude"], "long": h["longitude"]} for h in hubs}

    rows: list[dict[str, object]] = []

    # Cria um índice de rotas por cidade de destino para acelerar a busca
    routes_by_city = {}
    for r in routes:
    ...
    for i in range(1, total_rows + 1):
        ...
        # 6. Coleta de Coordenadas
        hub_id = route["origin_hub_id"]
        lat_origem = hubs_map[hub_id]["lat"]
        long_origem = hubs_map[hub_id]["long"]
        lat_destino = route["latitude_destino"]
        long_destino = route["longitude_destino"]
        ...
        rows.append({
            ...
            "latitude": lat_origem,             # Coordenada de Origem
            "longitude": long_origem,           # Coordenada de Origem
            "latitude_destino": lat_destino,    # Coordenada de Destino
            "longitude_destino": long_destino,  # Coordenada de Destino
            "is_late": is_late,
            "is_failed": is_failed,
...
if __name__ == "__main__":
    try:
        data = generate_rows(1500) 
        path = write_csv(data)
        print(f"Sucesso! fact_deliveries.csv gerado com coordenadas de origem e destino em: {path}")
